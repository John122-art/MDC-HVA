...
# KNARPE
# apply_q_rpe:为True时候，将rpe_q加到q上（不推荐这样做）
class AttentionRPE(nn.Module):
    ...
    def forward(
        ...
        ##如果tgt为None，执行自注意力，使用src计算q、k、v;如果tgt不为None，执行交叉注意力，使用src计算q，使用tgt计算k和v。
        if tgt is None:
            n_tgt = n_src
            # self-attention
        ...
        if attn_mask is not None:  # [n_batch, n_src, n_tgt], bool
            if attn_invalid_mask is None:
                ...
                attn_invalid_mask = attn_invalid_mask or attn_mask
        ...
        ##当 rpe 存在时，执行相对位置编码注意力。此时，q, k, v 与 rpe 结合计算注意力权重;否则判断是使用普通注意力还是KNN
        if rpe is None:
            if k.dim() == 3:
