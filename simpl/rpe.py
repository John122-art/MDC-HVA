...
from .transform_utils import torch_rad2rot, torch_pos2local, torch_rad2local
...
def get_tgt_knn_idx(
    ...
    idx_src = torch.arange(n_src)[None, :, None]  # [1, n_src, 1]就是0,1,2,...1024

    if 0 < n_tgt_knn < tgt_invalid.shape[1]:
        # [n_scene, n_src, n_tgt_knn]
        dist_knn, idx_tgt = torch.topk(rel_dist, n_tgt_knn, dim=-1, largest=False, sorted=False)#dist_knn（3,1024,36）表示距离   idx_tgt（3,1024,36）表示索引
        # [n_scene, n_src, n_tgt_knn]
        ...
        if rel_pose is None:
            ...
            rpe = rel_pose[idx_scene, idx_src, idx_tgt]##高级索引就是很神奇，idx_tgt在rel_Pose的第三维度，就用idx_tgt的第三个维度去索引  （3,1024,36,3）
    else:
        dist_knn = rel_dist
        tgt_invalid_knn = tgt_invalid.unsqueeze(1).expand(-1, n_src, -1)  # [n_scene, n_src, n_tgt](3,1024,36)
        rpe = rel_pose
        idx_tgt = None
