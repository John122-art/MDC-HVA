...
class ArgoDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        rot_lan = graph['lane_rot']
        lane_yaws = graph['lane_yaws']
       ...
       # scene_ctrs = torch.from_numpy(scene_ctrs)
        rpe_rot = torch.from_numpy(rpe_rot)
        rot_lan = torch.from_numpy(rot_lan)
        total_rot = torch.cat([rpe_rot, rot_lan], dim=0)

        lane_yaws = torch.from_numpy(lane_yaws)
        theta = torch.from_numpy(theta)
        total_yaw = torch.cat([theta, lane_yaws], dim=0)
        Rpe_xy = self.torch_pos2local(scene_ctrs.unsqueeze(0),scene_ctrs.unsqueeze(1),total_rot)
        total_yaw = self.torch_rad2local(total_yaw.unsqueeze(0), total_yaw, cast=False).unsqueeze(-1)
        rel_pos = torch.cat([Rpe_xy, total_yaw], dim=-1)
        ...
        data['RPE'] = rpes
        #data['RPE_ROT'] = rpe_rot
        data['RPE_XY'] = Rpe_xy
        data['TOTAL_YAW'] = total_yaw
        data['REL_POSE'] = rel_pos
    ...
    def torch_pos2local(self, in_pos, local_pos, local_rot) :
    ...
    def torch_rad2local(self,in_rad, local_rad, cast: bool = False) -> Tensor:
