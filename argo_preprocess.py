...
class ArgoPreproc():
    ...
    def process(self, seq_id, df):
        ...
        for traj in trajs_ori:
            ...
            act_orig2 = traj[self.args.obs_len - 5]
            act_vecyaw =act_orig - act_orig2
            Thet = np.arctan2(act_vecyaw[1], act_vecyaw[0])
        ...
        # collect data
        data = [[seq_id, city_name, orig, rot, ts, trajs, trajs_ctrs, trajs_vecs, pad_flags, lane_graph,trajs_rot,Theta]]
        headers = ["SEQ_ID", "CITY_NAME", "ORIG", "ROT", "TIMESTAMP",
                   "TRAJS", "TRAJS_CTRS", "TRAJS_VECS", "PAD_FLAGS", "LANE_GRAPH","TRAJS_ROT","THETA"]
    ...
    def get_lane_graph(self, seq_id, df, city_name, orig, rot, lane_ids):
        ...
        lane_ctrs, lane_vecs,lane_rot ,lane_yaws = [], [],[],[]
        ...
        for lane_id in lane_ids:
            ...
            for i in range(num_segs):
                ...
                ctrln[:, 0:2] = (ctrln[:, 0:2] - orig).dot(rot)#將道路中心线从全局坐标系转换到局部坐标系

                anch_pos = np.mean(ctrln, axis=0) #计算车道段的平均位置和 方向向量
                anch_vec = (ctrln[-1] - ctrln[0]) / np.linalg.norm(ctrln[-1] - ctrln[0])
                yaw_angle = np.arctan2(anch_vec[1], anch_vec[0])
                lane_yaws.append(yaw_angle)
                ...
                lane_rot.append(anch_rot)

                ctrln[:, 0:2] = (ctrln[:, 0:2] - anch_pos).dot(anch_rot)#将道路中心线点对齐到车道段的方向上
        ...
        graph['lane_rot'] = np.array(lane_rot).astype(np.float32)
        graph['lane_yaws'] = np.array(lane_yaws).astype(np.float32)
